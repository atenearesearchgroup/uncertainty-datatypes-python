...
class uenum:
    ...
    def __init__(self, literals: Iterable[str], conf: Iterable[float] = None):
        if conf is None:
            if isinstance(literals, dict):
                ...
                for i in range(len(literals)):
                    self._elements[literals[i].string] = literals[i].c
        else:
            if len(literals) != len(conf):
                raise ValueError("Sizes of the parameters 'literals' and 'conf' are not the same.")

            self._elements = {}
            for i in range(len(literals)):
                self._elements[literals[i]] = conf[i]
